...
tmpHTML += tHeadClose;
    ...
    "destroy": true,
    "ajax": {
        "url": "/EmployeeMaster/GetIndexDataOnLoad",
        "type": "POST",
        "datatype": "json",
        "data": Postdata
    },
...
function Delete(empMasterId)
{   

    $.ajax({
        type: 'POST',
        url: '/EmployeeMaster/Delete',
        data: { ID: empMasterId, groupId: sessionStorage.getItem('groupId'), sessionId: sessionStorage.getItem('sessionYear'), schoolId: sessionStorage.getItem('schoolId'), deletedBy: sessionStorage.getItem('CUserId')},
        dataType: "json",
...
function Returnobj(selectColumnToGrid) {
    ...
    var objaction = {
        ...
        data: null, render: function (data, type, row) {
            return "<button class='btn btn-primary btn-xs edit' id='Edit' value='Edit' onclick=EditData('" + row.EmployeeMasterID + "') ><i class='fa fa-pencil-square-o' aria-hidden='true'></i></button> <button id='Delete' value='Delete' onclick=DeleteData('" + row.CUserId + "') class='btn btn-danger btn-xs delete'><i class='fa fa-trash' aria-hidden='true'></i></button>";
