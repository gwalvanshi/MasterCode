...
namespace GEE_Web.Areas.Admin.Controllers
{
    public class BookMasterController : BaseController
    {
        ...
        BookMasterService service = new BookMasterService();
        ...
        {
            BookMasterGridViewModel collection = new BookMasterGridViewModel();
            ...
            var list = await service.GetAllBookMasterData(stringContent);
            if (list.Count > 0)
            {
        ...
        /// <returns></returns>
        [HttpPost]
        public async Task<JsonResult> Create(BookMasterViewModel collection)
        {
            try
            {
